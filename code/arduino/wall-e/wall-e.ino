...
#define PWM_SPEED_L_PIN 10           // Motor PWM pins
#define PWM_SPEED_R_PIN 11
#define BRAKE_L_PIN  9               // Motor brake pins
#define BRAKE_R_PIN  8
#define SERVO_ENABLE_PIN 7          // Servo shield output enable pin
...
#define BAT_L
  ...
  #define OLED
	  ...
	  #include <U8g2lib.h>
	  U8G2_SSD1306_128X64_NONAME_1_HW_I2C u8g2(U8G2_R0, 10);
...
int preset[][2] =  {{478,262},  // head rotation
                    {195,518},  // neck top
                    {290,570},  // neck bottom
                    {360,250},  // eye right
                    {390,450},  // eye left
...
void readSerial() {
  ...
  if (mySerial.available()) {
    ...
    Serial.print("Data from mySerial: ");
    Serial.println(inchar);
  }
  // 如果 Serial 没有数据，尝试从 mySerial 读取
  else if (Serial.available()) {
    inchar = Serial.read();
    Serial.print("Data from Serial: ");
    Serial.println(inchar);
  }
